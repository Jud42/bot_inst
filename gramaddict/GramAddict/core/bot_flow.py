...
def start_bot(**kwargs):
    ...
    users = (
        [configs.args.username] if isinstance(configs.args.username, str) 
        else configs.args.username or [None]
    )
    password = (
        [configs.args.password] if isinstance(configs.args.password, str) 
        else configs.args.password or [None]
    )
    if (
        (users == [None] and password != [None]) or 
        (users != [None] and password == [None]) or
        len(users) != len(password)
    ):
        logger.error(
            "Problems with username or password field in config.yml\n"
            "Format required:\n"
    ...
    index = 0
    break_ = False
    for configs.args.username, configs.args.password in zip(users, password):
        index += 1
        while True:
            ...
            try:
                ...
                if not log_in and (configs.args.username and configs.args.username is not None):
                    success = account_view.changeToUsername(configs.args.username, configs.args.password)
                    if not success:
                        logger.error(
            ...
            if users is None or len(users) == index:
                # turn off bot
